...
class Tweet(snscrape.base.Item):
    ...
    username = snscrape.base._DeprecatedProperty(
        'username', lambda self: getattr(
            self.user, 'username', None), 'user.username')
    outlinks = snscrape.base._DeprecatedProperty(
        'outlinks',
        lambda self: [
            x.url for x in self.links] if self.links else [],
        'links (url attribute)')
    ...
    tcooutlinks = snscrape.base._DeprecatedProperty(
        'tcooutlinks',
        lambda self: [
            x.tcourl for x in self.links] if self.links else [],
...
class UnifiedCard(Card):
    componentObjects: typing.Dict[UnifiedCardComponentKey,
                                  'UnifiedCardComponentObject']
    destinations: typing.Dict[UnifiedCardDestinationKey,
                              'UnifiedCardDestination']
    media: typing.Dict[UnifiedCardMediumKey, Medium]
    apps: typing.Optional[typing.Dict[UnifiedCardAppKey,
                                      typing.List['UnifiedCardApp']]] = None
...
@dataclasses.dataclass
class UnifiedCardTwitterCommunityDetailsComponentObject(
        UnifiedCardComponentObject):
    name: str
    theme: str
    membersCount: int
...
class User(snscrape.base.Item):
    ...
    # Raw description with the URL(s) intact
    rawDescription: typing.Optional[str] = None
    # Description as it's displayed on the web interface with URLs replaced
    renderedDescription: typing.Optional[str] = None
...
class _CLIGuestTokenManager(GuestTokenManager):
    def __init__(self):
        ...
        if not cacheHome or not os.path.isabs(cacheHome):
            ...
            # On POSIX-compliant systems, the XDG Base Directory specification
            # is followed exactly since ~ expands to $HOME if it is present.
        ...
        if not os.path.isdir(dir):
            ...
            # This ensures that the XDG_CACHE_HOME is created with the right
            # permissions.
    ...
    def _locked_load(self):
        ...
        with open(self._file, 'r') as fp:
            try:
                ...
                _logger.warning(
                    f'Malformed guest token file {self._file}: {e!s}')
                self._locked_delete()
                return None
        if o.get('version') != 1:
            _logger.warning(
                f'Outdated version of guest token file {self._file}')
            self._locked_delete()
            return None
        return o
    ...
    def _read(self):
        ...
        validTokens = [
            token for token,
            t in o['tokens'].items() if t['setTime'] >= setTimeThreshold and t.get(
                'blockedUntil',
                0) < currentTime]
        if not validTokens:
            return None
    ...
    def _write(self):
        with self._lock:
            ...
            o['tokens'] = {
                token: details for token,
                details in o['tokens'].items() if details['setTime'] >= setTimeThreshold}
    ...
    def token(self, token):
        super(type(self), type(self)).token.__set__(
            self, token)  # https://bugs.python.org/issue14965
        self._write()
...
class _TwitterTLSAdapter(snscrape.base._HTTPSAdapter):
    def init_poolmanager(self, *args, **kwargs):
        # FIXME: When urllib3 2.0.0 is out and can be required, this should use
...
class TwitterAuthContext(typing.NamedTuple):
...
class TwitterSessionManager:
    ...
    def __init__(
        self,
        contexts: typing.List[TwitterAuthContext],
        # authorizationHeader: str,
        # cookies: dict[str, str],
        # csrfToken: typing.Optional[str] = None,
        ...
        self._current_context = collections.defaultdict(lambda: 0)
        self._daily_rate_limits = collections.defaultdict(
            lambda: collections.defaultdict(dict)
        )
        self._context_info = collections.defaultdict(
            lambda: collections.defaultdict(dict)
        )

        # self._authorizationHeader = authorizationHeader
        # self._cookies = cookies
        # self._csrfToken = csrfToken or cookies.get('ct0')
    ...
    def clean_path(self, path):
        return urllib.parse.urlsplit(
            urllib.parse.urljoin('https://twitter.com', path)).path
    ...
    #     return self._context_info[self._current_context]
    
    # def next_context(self, next_by: int = 1):
    #     return self._context_info[(self._current_context + next_by) % len(self._contexts)]

    # def increment_context(self):
    #     self._current_context = (self._current_context + 1) % len(self._contexts)

    def get_api_headers(self, path: typing.Optional[str] = None):
        path = self.clean_path(path) if path else '__default__'

        context = self.get_context(
            self._current_context[path]
        )
        ...
        return {
            ...
            'Cookie': '; '.join(
                f'{k}={v}' for k, v in context.cookies.items()
            ),
    ...
    def on_before_request(
        ...
        # default_reset_in = int(datetime.datetime.now().timestamp()) + 1000
        ...
        _logger.info(context_info[path])

        # rate_limit = context.get(
        #     path, {}).get('limit', default_limit)

        rate_limit_remaining = context_info.get(
            path, {}).get('remaining', default_limit)
        
        _logger.info(f'rate_limit_remaining {rate_limit_remaining}')

        # rate_limit_reset = context.get(
        #     path, {}).get('reset', default_reset_in)
        ...
        if rate_limit_remaining == 0:
            ...
            idx_reset_at = {
                ix: self.get_context_info(ix=ix).get(path, {}).get('reset', -1)
                for ix in range(len(self._contexts))
            }

            _logger.info(f'idx_reset_at {idx_reset_at}')
            ...
            resets_in_seconds = (min_reset_at - int(
        ...
        # while True:
        #     # if inc_contex
        #     context = self.next_context(inc_context - 1)
        ...
        # _logger.info((
        #     f'on_before_request: {rate_limit_remaining}/{rate_limit}'
        #     f' - resets in  {resets_in_seconds}'
        # ))
    ...
    def on_paginate(
        ...
        path = self.clean_path(path)
        self._context_info[self._current_context[path]]['cursor'] = cursor
    ...
    def on_response(
        ...
        if response.status_code == 429:
            ...
            remaining = int(response.headers.get('x-rate-limit-remaining')) \
                if response.headers.get('x-rate-limit-remaining') else 0

        self._context_info[self._current_context[path]][path] = {
...
class _TwitterAPIScraper(snscrape.base.Scraper):
    ...
    #     return {
    #         **self._sessionManager.get_api_headers(),
    #         'Referer': self._baseUrl,
    #     }

    def _get_api_headers(self, path):
        return {
            **self._sessionManager.get_api_headers(path=path),
            'Referer': self._baseUrl,
        }
    ...
    def _get_api_url(self, method):
        return f'https://twitter.com/i/api/graphql/{self._methodKeyMap[method]}/{method}'

    # def _check_missing_features(self, method, features):
    #     missingFeatures = set(features.keys()) - set(self._methodFeatures[method])
    #     if missingFeatures:
    #         raise snscrape.base.ScraperException(
    #             f'Missing features for method {method}: {missingFeatures}')

    def _get_features(self, method, features):
        _features = self._methodFeatures[method].copy()
        _features.update(features)
        return _features

    def _check_api_response(self, r, apiType, instructionsPath):
        self._sessionManager.on_response(r)
        ...
        # Pass the already-parsed object outwards so it doesn't need to be
        # decoded twice.
        r._snscrapeObj = obj
        if apiType is _TwitterAPIType.GRAPHQL and 'errors' in obj:
            msg = 'Twitter responded with an error: ' + \
                ', '.join(
            ...
            if instructions:
                # Emit a warning if there are instructions since it could
    ...
    def _iter_api_data(
        ...
        # Once the top pagination is exhausted, the bottomCursorAndStop is used
        ...
        while True:
            ...
            # No data format test, just a hard and loud crash if anything's
            # wrong :-)
            newCursor = None
            promptCursor = None
            ...
            for instruction in instructions:
                ...
                for entry in entries:
                    ...
                    entryCursor, entryCursorStop = cursorContent['value'], cursorContent.get(
                        'stopOnEmptyResponse', None)
                    if entry['entryId'] == f'sq-cursor-{dir}' or entry['entryId'].startswith(
                            f'cursor-{dir}-'):
                        ...
                        # E.g. 'offensive' replies and 'Show more replies'
                        # button
                        ...
                        newBottomCursorAndStop = (
                            entryCursor, entryCursorStop or False)
            ...
            if newCursor == cursor and entryCount == 0:
                ...
                # When this happens, retry the same cursor up to the retries
                # setting.
    ...
    def _count_tweets_and_users(self, entries):
        return sum(entry['entryId'].startswith('sq-I-t-') or entry['entryId'].startswith(
            'tweet-') or entry['entryId'].startswith('user-') for entry in entries)

    def _get_tweet_id(self, tweet):
        return tweet['id'] if 'id' in tweet else int(tweet['id_str'])

    def _make_tweet(self, tweet, user, retweetedTweet=None,
                    quotedTweet=None, card=None, noteTweet=None, **kwargs):
                ...
                for u in entities['user_mentions']:
                    if u['id_str'] == tweet['in_reply_to_user_id_str']:
                        kwargs['inReplyToUser'] = User(
                            username=u['screen_name'], id=u['id'] if 'id' in u else int(
                                u['id_str']), displayname=u['name'])
        ...
        if entities.get('user_mentions'):
            kwargs['mentionedUsers'] = [
                User(
                    ...
                    id=u['id'] if 'id' in u else int(
                        u['id_str']),
                    displayname=u['name']) for u in entities['user_mentions']]

        # https://developer.twitter.com/en/docs/tutorials/filtering-tweets-by-location
        if tweet.get('coordinates'):
            # coordinates root key (if present) presents coordinates in the
            # form [LONGITUDE, LATITUDE]
            if (coords := tweet['coordinates']
                    ['coordinates']) and len(coords) == 2:
        ...
        if tweet.get('place'):
            ...
            if 'coordinates' not in kwargs and tweet['place'].get('bounding_box') and (
                ...
                # Take the first (longitude, latitude) couple of the "place
                # square"
        ...
        if card:
            ...
            if hasattr(card, 'url') and '//t.co/' in card.url:
                ...
                # Retweets inherit the card but not the outlinks; try to get
                # them from the retweeted tweet instead in that case.
                candidates = []
                if 'links' in kwargs:
    ...
    def _make_medium(self, medium, tweetId):
        if medium['type'] == 'photo':
            if '?format=' in medium['media_url_https'] or '&format=' in medium['media_url_https']:
                return Photo(
                    previewUrl=medium['media_url_https'], fullUrl=medium['media_url_https'])
    ...
    def _make_card(self, card, apiType, tweetId):
        ...
        for o in card['legacy'].get('user_refs_results', []):
            if 'result' not in o:
                _logger.warning(
                    f'Empty user ref object in card on tweet {tweetId}')
                continue
            ...
            if user.id in userRefs:
                if userRefs[user.id] != user:
                    _logger.warning(
                        f'Duplicate user {user.id} with differing data in card on tweet {tweetId}')
                continue
            userRefs[user.id] = user

        messyBindingValues = ((x['key'], x['value'])
                              for x in card['legacy']['binding_values'])
        for key, value in messyBindingValues:
            if 'type' not in value:
                # Silently ignore creator/site entries since they frequently
                # appear like this.
            ...
            if value['type'] == 'STRING':
                ...
                if key.endswith('_datetime_utc'):
                    bindingValues[key] = datetime.datetime.strptime(
                        bindingValues[key], '%Y-%m-%dT%H:%M:%SZ').replace(tzinfo=datetime.timezone.utc)
        ...
        if cardName in ('summary', 'summary_large_image',
                        'app', 'direct_store_link_app'):
            keyMap = {
                'title': 'title',
            ...
            if cardName in ('app', 'direct_store_link_app'):
                ...
                return AppCard(
                    **snscrape.utils.dict_map(bindingValues, keyMap))
            else:
            ...
            if cardName.endswith('_image'):
                ...
                kwargs['medium'] = Video(
                    ...
                    variants=variants,
                    duration=int(
                        bindingValues['content_duration_seconds']))

            return PollCard(**kwargs)
        elif cardName == 'player':
            return PlayerCard(**snscrape.utils.dict_map(bindingValues, {
                              'title': 'title', 'description': 'description', 'card_url': 'url', 'player_image_original': 'imageUrl', 'site': 'siteUser'}))
            ...
            if 'broadcaster_twitter_id' in bindingValues:
                if int(bindingValues['broadcaster_twitter_id']) in userRefs:
                    kwargs['broadcaster'] = userRefs[int(
                        bindingValues['broadcaster_twitter_id'])]
                else:
            ...
            kwargs['photo'] = Photo(
                ...
                fullUrl=bindingValues.get('event_thumbnail_original') or bindingValues['event_thumbnail'])
            ...
            return SpacesCard(
                **snscrape.utils.dict_map(bindingValues, {'card_url': 'url', 'id': 'id'}))
            ...
            # cta_1, 3, and 4 should mean 'Message us', 'Send a private
            # message', and 'Send me a private message', but the correct
            # mapping is currently unknown.
            ...
            if bindingValues['cta'] not in ctas:
                ...
                return
            return MessageMeCard(**snscrape.utils.dict_map(bindingValues, {
                                 'recipient': 'recipient', 'card_url': 'url'}), buttonText=ctas[bindingValues['cta']])
            ...
            if 'type' in o:
                ...
                _logger.warning(
                    f'Unsupported unified_card type on tweet {tweetId}')
                return
            ...
            for k, v in o['component_objects'].items():
                if v['type'] == 'details':
                    ...
                    co = UnifiedCardMediumComponentObject(
                        mediumKey=v['data']['id'], destinationKey=v['data']['destination'])
                elif v['type'] == 'button_group':
                    if not all(
                            b['type'] == 'cta' for b in v['data']['buttons']):
                        _logger.warning(
                            f'Unsupported unified_card button_group button type on tweet {tweetId}')
                        return
                    buttons = [UnifiedCardButton(text=b['action'][0].upper() + re.sub('[A-Z]',
                                                                                      lambda x: f' {x[0]}',
                                                                                      b['action'][1:]),
                    ...
                    media = [
                        UnifiedCardSwipeableMediaMedium(
                            mediumKey=m['id'],
                            destinationKey=m['destination']) for m in v['data']['media_list']]
                    ...
                    co = UnifiedCardAppStoreComponentObject(
                        appKey=v['data']['app_id'], destinationKey=v['data']['destination'])
                    ...
                    co = UnifiedCardTwitterCommunityDetailsComponentObject(
                        ...
                        destinationKey=v['data']['destination'],
                        membersFacepile=[
                            self._user_to_user(u) for u in map(
                                o['users'].get,
                                v['data']['members_facepile']) if u],
                    )
                else:
                    _logger.warning(
            ...
            if 'app_store_data' in o:
                ...
                for k, v in o['app_store_data'].items():
                    ...
                    for var in v:
                        ...
                        vKwargs = {
                            kwarg: var[key] for key,
                            kwarg in vKwargsMap.items() if key in var}
            ...
            if 'layout' in o:
                if o['layout']['type'] == 'swipeable':
                    kwargs['swipeableLayoutSlides'] = [
                        UnifiedCardSwipeableLayoutSlide(
                            mediumComponentKey=v[0],
                            componentKey=v[1]) for v in o['layout']['data']['slides']]
                elif o['layout']['type'] == 'collection':
                    kwargs['collectionLayoutSlides'] = [
                        UnifiedCardCollectionLayoutSlide(
                            detailsComponentKey=v[0],
                            mediumComponentKey=v[1]) for v in o['layout']['data']['slides']]
                else:
                    _logger.warning(
            ...
            if card.swipeableLayoutSlides and not all(
                    s.mediumComponentKey in card.componentObjects and s.componentKey in card.componentObjects for s in card.swipeableLayoutSlides):
                missingParts.add('components')
            if any(c.destinationKey not in card.destinations for c in card.componentObjects.values(
            ) if hasattr(c, 'destinationKey')):
                missingParts.add('destinations')
            if any(b.destinationKey not in card.destinations for c in card.componentObjects.values(
            ) if isinstance(c, UnifiedCardButtonGroupComponentObject) for b in c.buttons):
                missingParts.add('destinations')
            mediaKeys = []
            ...
            mediaKeys.extend(
                d.mediumKey for d in card.destinations.values() if d.mediumKey is not None)
            mediaKeys.extend(a.iconMediumKey for l in (card.apps.values(
            ) if card.apps is not None else []) for a in l if a.iconMediumKey is not None)
            if any(k not in card.media for k in mediaKeys):
                missingParts.add('media')
            if any(c.appKey not in card.apps for c in card.componentObjects.values(
            ) if hasattr(c, 'appKey')):
                missingParts.add('apps')
            if any(d.appKey not in card.apps for d in card.destinations.values(
        ...
        _logger.warning(
            f'Unsupported card type on tweet {tweetId}: {cardName!r}')
    ...
    def _make_edit_state(self, editControl):
        ...
        return EditState(
            ...
            editableUntilDate=datetime.datetime.fromtimestamp(
                int(editControl['editable_until_msecs']) / 1000, tz=datetime.timezone.utc),
    ...
    def _make_tombstone(self, tweetId, info):
        if tweetId is None:
            raise snscrape.base.ScraperException(
                'Cannot create tombstone without tweet ID')
        if info and (text := info.get('richText', info['text'])):
            return Tombstone(
                id=tweetId,
                text=text['text'],
                textLinks=[TextLink(text=text['text'][x['fromIndex']:x['toIndex']],
                                    url=x['ref']['url'],
                                    tcourl=None,
                                    indices=(x['fromIndex'],
                                             x['toIndex'])) for x in text['entities']],
            )
        else:
    ...
    def _graphql_timeline_tweet_item_result_to_tweet(
        ...
        **kwargs
        ...
        user = self._graphql_user_results_to_user(
            result['core']['user_results'], userId=int(
        ...
        if 'quoted_status_result' in result:
            if 'result' not in result['quoted_status_result']:
                ...
                kwargs['quotedTweet'] = TweetRef(
                    int(tweet['quoted_status_id_str']))
            else:
            ...
            # Omit the TweetRef if this is a retweet and the quoted tweet ID
            # matches the tweet quoted in the retweeted tweet.
            if tweet['quoted_status_id_str'] != tweet.get('retweeted_status_result', {}).get(
                    'result', {}).get('quoted_status_result', {}).get('result', {}).get('rest_id'):
                kwargs['quotedTweet'] = TweetRef(
                    id=int(tweet['quoted_status_id_str']))
        if 'card' in result:
            kwargs['card'] = self._make_card(
                result['card'],
                _TwitterAPIType.GRAPHQL,
                self._get_tweet_id(tweet))
    ...
    def _graphql_timeline_instructions_to_tweets(
        self,
        instructions,
        includeConversationThreads=False,
        **kwargs
    ):
        for instruction in instructions:
                ...
                if entry['entryId'].startswith('tweet-'):
                    ...
                    if entry['content']['entryType'] == 'TimelineTimelineModule':
                        for item in reversed(entry['content']['items']):
                            if not item['entryId'].startswith(entry['entryId'].split(
                                    'ion-', 1)[0] + 'ion-') or '-tweet-' not in item['entryId']:
                                raise snscrape.base.ScraperException(
                ...
                # TODO show more cursor?
                    ...
                    for item in entry['content']['items']:
                        if item['entryId'].startswith(
                            ...
                            if item['entryId'][len(
                                    entry['entryId']) + 7:].strip('0123456789'):
                                _logger.warning(
                                    f'Skipping promoted tweet entry {item["entryId"]}')
                                continue
                            tweetId = int(
    ...
    def _render_text_with_urls(self, text, urls):
        ...
        assert all(url['indices'][1] <= nextUrl['indices'][0]
                   for url, nextUrl in zip(urls, urls[1:])), 'broken URL indices'
        for url, nextUrl in itertools.zip_longest(urls, urls[1:]):
            if 'display_url' in url:
                out.append(url['display_url'])
            out.append(text[url['indices'][1]: nextUrl['indices']
                       [0] if nextUrl is not None else None])
        return ''.join(out)

    def _user_to_user(self, user, id_=None, **kwargs):
        kwargs['username'] = user['screen_name']
        kwargs['id'] = id_ if id_ else user['id'] if 'id' in user else int(
            user['id_str'])
        ...
        kwargs['created'] = email.utils.parsedate_to_datetime(
            user['created_at'])
        ...
        if user.get('url'):
            ...
            kwargs['link'] = TextLink(
                text=entity.get('display_url'), url=entity.get(
                    'expanded_url', user['url']), tcourl=user['url'], indices=tuple(
                    entity['indices']))
        kwargs['profileImageUrl'] = user['profile_image_url_https']
        kwargs['profileBannerUrl'] = user.get('profile_banner_url')
        if 'label' not in kwargs and (labelO := user.get(
                'affiliates_highlighted_label', {}).get('label')):
    ...
    def _graphql_user_results_to_user_ref(self, obj, userId=None):
        ...
        if 'result' in obj and obj['result']['__typename'] == 'UserUnavailable' and 'unavailable_message' in obj['result']:
            ...
            kwargs['textLinks'] = [TextLink(text=kwargs['text'][x['fromIndex']:x['toIndex']], url=x['ref']['url'], tcourl=None, indices=(
                x['fromIndex'], x['toIndex'])) for x in obj['result']['unavailable_message']['entities']]
...
class TwitterSearchScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        if self._mode is TwitterSearchScraperMode.USER:
            raise snscrape.base.ScraperException(
                'User searches currently unsupported')
    ...
    def _cli_setup_parser(cls, subparser):
        ...
        subparser.add_argument(
            'query',
            type=snscrape.utils.nonempty_string_arg('query'),
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        ...
        subparser.add_argument(
            'user',
            type=user,
            help='A Twitter username (without @)')

    @classmethod
    def _cli_from_args(cls, args):
        return cls._cli_construct(args, user=int(
            args.user) if args.isUserId else args.user)
...
class TwitterProfileScraper(TwitterUserScraper):
    ...
    def get_items(self, count: int = 40, cursor: str | None = None):
        ...
        for obj in self._iter_api_data(
            ...
            if not gotPinned:
                for instruction in instructions:
                    if instruction['type'] == 'TimelinePinEntry':
                        ...
                        yield self._graphql_timeline_tweet_item_result_to_tweet(
                            instruction['entry']['content']['itemContent']['tweet_results']['result'], tweetId=tweetId, pinned=True
                        )
            tweets = list(
            ...
            # those
            for tweet in tweets:
                if getattr(getattr(tweet, 'user', None),
                           'id', userId) != userId:
                    continue
                yield tweet
...
class TwitterHashtagScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            'hashtag',
            type=snscrape.utils.nonempty_string_arg('hashtag'),
...
class TwitterCashtagScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            'cashtag',
            type=snscrape.utils.nonempty_string_arg('cashtag'),
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def __init__(self, tweetId, *,
        ...
        self._mode = mode
        super().__init__(
            f'https://twitter.com/i/web/status/{self._tweetId}',
            **kwargs)
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            ...
            for instruction in obj['data']['threaded_conversation_with_injections_v2']['instructions']:
                ...
                for entry in instruction['entries']:
                    if entry['entryId'] == f'tweet-{self._tweetId}' and entry['content'][
                            'entryType'] == 'TimelineTimelineItem' and entry['content']['itemContent']['itemType'] == 'TimelineTweet':
            ...
            for obj in self._iter_api_data(url, _TwitterAPIType.GRAPHQL, params, paginationParams,
                ...
                hasModeratedReplies = hasModeratedReplies or self._has_moderated_replies(
                    obj, self._tweetId)
        ...
        for instruction in obj['data']['threaded_conversation_with_injections_v2']['instructions']:
            ...
            for entry in instruction['entries']:
                if entry['entryId'] == f'tweet-{tweetId}' and entry['content']['entryType'] == 'TimelineTimelineItem' and entry['content']['itemContent']['itemType'] == 'TimelineTweet':
                    return entry['content']['itemContent'].get(
                        'hasModeratedReplies', False)
        return False
    ...
    def _get_moderated_replies(self, tweetId):
        ...
        paginationParams = {
            'variables': paginationVariables,
            'features': features}
        url = self._get_api_url('ModeratedTimeline')
        instructionsPath = [
            'data',
            'tweet',
            'result',
...
class TwitterListScraper(_TwitterAPIScraper):
    _method = 'ListByRestId'
    ...
    def get(self):
        ...
        _logger.info(self._get_api_url(self._method))

        _logger.info(params)

        # return self, self._get_api_url('ListByRestId'), params
        ...
        return List(
            id=list_obj['id_str'],
            name=list_obj['name'],
            created_at=datetime.datetime.fromtimestamp(list_obj['created_at']/1000),
            following=list_obj['following'],
            is_member=list_obj['is_member'],
            description=list_obj['description'],
            member_count=list_obj['member_count'],
            subscriber_count=list_obj['subscriber_count'],
            mode=list_obj['mode'],
            user=user
        )
...
class TwitterListPostsScraper(TwitterSearchScraper):
    ...
    def _cli_setup_parser(cls, subparser):
        subparser.add_argument(
            'list',
            type=snscrape.utils.nonempty_string_arg('list'),
...
class TwitterCommunityScraper(_TwitterAPIScraper):
    ...
    def __init__(self, communityId, **kwargs):
        ...
        super().__init__(
            f'https://twitter.com/i/communities/{self._communityId}',
            **kwargs)
    ...
    def _get_entity(self):
        ...
        params['features'] = self._get_features(
            'CommunityFetchOneQuery', params['features']
        )

        obj = self._get_api_data(
            self._get_api_url('CommunitiesFetchOneQuery'),
            _TwitterAPIType.GRAPHQL,
            params=params,
            instructionsPath=[
                'data',
        ...
        return Community(
            ...
            admin=self._graphql_user_results_to_user(
                community['admin_results']),
            creator=self._graphql_user_results_to_user(
                community['creator_results']),
            membersFacepile=[self._graphql_user_results_to_user(
                m) for m in community['members_facepile_results']],
            ...
            bannerUrl=community.get('custom_banner_media', community['default_banner_media'])[
    ...
    def get_items(self):
        ...
        features = self._get_features(
            'CommunitiesTimelineQuery', features
        )
        ...
        for obj in self._iter_api_data(
            self._get_api_url('CommunitiesTimelineQuery'), 
             _TwitterAPIType.GRAPHQL,
             params, paginationParams, instructionsPath=['data', 'communityResults', 'result', 'community_timeline', 'timeline', 'instructions']):
            if obj['data']['communityResults']['result']['__typename'] == 'CommunityUnavailable':
                raise snscrape.base.EntityUnavailable('Community unavailable')
...
class TwitterUsersScraper(_TwitterAPIScraper):
    ...
    def __init__(self, userIds, **kwargs):
        ...
        super().__init__(
            f'https://twitter.com/i/user/{self._userIds[0]}',
            **kwargs)
    ...
    def get_items(self):
        ...
        features = self._get_features('UsersByRestId', features)

        obj = self._get_api_data(
            self._get_api_url('UsersByRestId'),
...
__getattr__, __dir__ = snscrape.utils.module_deprecation_helper(
    __all__, DescriptionURL=TextLink)
